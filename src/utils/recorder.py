"""
Utility functions and global recorder instance
"""
...
""")
    ...
    def dump_llm_interaction(self, prompt: str = "", system_prompt: str = "", 
                           thinking: str = "", response: str = "", model_name: str = "Unknown",
                           max_new_tokens: int = 0, truncated: bool = False,
                           agent_name: str = None, token_info: Dict[str, Any] = None):
        """Record LLM interactions with full context (flexible parameter support)"""
        # Support both old and new calling patterns
        agent = agent_name or model_name
        ...
        with open(self.log_file, "a", encoding="utf-8") as f:
            f.write(f"[{datetime.now().strftime('%H:%M:%S')}] LLM INTERACTION - {agent}\n")
            if model_name != agent:
            ...
            if max_new_tokens:
                f.write(f"Max Tokens: {max_new_tokens}, Truncated: {truncated}\n")
            if token_info:
...
def get_global_recorder() -> Record:
    """Get or create the global recorder instance"""
    global _global_recorder
    if _global_recorder is None:
        _global_recorder = Record(truncate_content=False)
    return _global_recorder


def set_global_recorder(recorder: Record):
    """Set the global recorder instance"""
    global _global_recorder
    _global_recorder = recorder


def create_recorder(session_name: str = None, truncate_content: bool = False) -> Record:
    """Create a new recorder instance"""
    return Record(session_name=session_name, truncate_content=truncate_content)
